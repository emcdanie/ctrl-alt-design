...
export default function ContactSection() {
  ...
  const fieldClass = (hasError: boolean) =>
    `w-full bg-white/5 border rounded-xl px-4 py-3 text-[#EDE8DF] text-[16px] placeholder-[#8A8480] outline-none transition-colors focus:border-white/30 ${
    ...
    <footer id="contact" className="bg-[#1A1814] text-[#EDE8DF] pt-20 pb-10 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mb-16">
          ...
          <div className="flex flex-col justify-between gap-10">
            <div>
              ...
              <p className="text-[16px] text-[#8A8480] leading-relaxed max-w-sm">
            ...
            <div className="grid grid-cols-2 gap-8">
              {Object.entries(footerNav).map(([section, links]) => (
                <div key={section}>
                  <p className="text-[13px] font-medium uppercase tracking-widest text-[#5A5450] mb-3">
                    {section}
                  </p>
                  <ul className="space-y-2">
                    {links.map((link) => (
                      <li key={link.label}>
                        <a
                          href={link.href}
                          className="text-[15px] text-[#8A8480] hover:text-[#EDE8DF] transition-colors"
                        >
                          {link.label}
                        </a>
                      </li>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <p className="text-[13px] text-[#5A5450]">
