...
function sceneFile() {
  ...
  while (x < 1760) {
    /* a column holding a lavender duplicate is wide enough for its
       longest name ("◆ _old_Users/Edit form", ~9 units a Mono letter at
       13 with Linux's wider rendering): the label ran past its highlight
       (audit:contain, A8) */
    const w = dupCols.includes(colI) ? Math.max(ri(120, 210), 200) : ri(120, 210);
    let y = ri(14, 50);
    while (y < 640) {
      const h = ri(70, 190);
      ...
      /* the highlight has headroom above its label (A8) */
      o += `<g class="${isDup ? "k k1 dup" + colI : ""}">` + (isDup ? R(x - 5, y - 22, w + 10, h + 28, 10, "hlr", "opacity:.45") : "") + L(x, y - 5, nm) + R(x, y, w, h, 6, "s", "opacity:.55") + kinds[kind](x, y, w, h) + "</g>";
      if (isDup) dupPlaced++;
