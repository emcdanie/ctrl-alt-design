...
const study: CaseStudy = {
  ...
  blocks: [
    ...
    {
      kind: "summary",
      context: "I opened the Figma file and the code side by side. The button in Figma said 'Primary, Large.' The button in Storybook said 'variant: action, size: lg.' Same component. Different names. Different assumptions. And this was one of the simpler ones. Somewhere in the gap between those two descriptions, a year of quiet drift had accumulated, and nobody had noticed because the system still looked right, even though it no longer said the same thing in both places.",
      approach: "Working as part of the Maker Program, the challenge wasn't to design a system and hand it off. It was to understand an existing, principled system deeply enough to contribute at the component and token level, to close the gap between Figma and code, and to explore whether AI tooling could make that kind of structural investigation fundamentally faster.",
      outcome: "Figma and Storybook components aligned across the system. Token layer documented with explicit primitive → semantic → component chain. MCP workflow established for ongoing system investigation.",
    },
    {
      kind: "section",
      eyebrow: "BACKGROUND",
    ...
    /* TODO(elleta): decision 02 `why` slot is empty (same as 01). */
    {
      kind: "decision",
      index: "02",
      title: "Using Claude MCP for System Investigation",
      children: [
...
/* TODO(elleta): imagery (Pass C sweep of the local BFW workspace found
 * nothing both public and on-topic, so nothing was used):
 * - eddie-design-system repo is PRIVATE on GitHub: no Eddie visuals
 *   until Brad OKs them or the repo goes public.
 * - the AI & Design Systems course social card is public marketing but
 *   promotes the course, not the Maker Program work: off-topic here.
 * - Artifacts thumbnails (references library, automation command
 *   center) are internal tooling previews: ambiguous, not used.
 * The public MCP-session poster (Pass A) is the one imagery upgrade. */
